...
class PyWordCounter(object):
    ...
    def handle_config(self, base_dir):
        ...
        def tolist(v):
            return v and [i.strip() for i in v.split(",")] or v
        ...
        for proc in set(sum([v for v in procs.values() if v], [])):
            proc_funcs[proc] = __import__(
                proc, globals(), locals(), ["pywordcountplugin"]
            ).pywordcountplugin
    ...
    def setup_cli_parser(self):
        ...
        parser.add_option("-t", "--type", dest="type", help="set TYPE", metavar="TYPE")
        return parser
    ...
    def count_words(self, text):
        def ors(l):
            return r"|".join([re.escape(c) for c in l])
